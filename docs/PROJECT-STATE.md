...
_Last updated: 2026-06-04 (mobile/feature overhaul Phases 1–6 merged, PRs #25–#31; migrations through `0012`)._
...
## Supabase
- **dev — `nzx-dev`**, ref **`mlbskwkjwbdewbmoagav`**, region East US. Touched by local dev + Vercel previews + CI + `deploy-dev`. Migrations `0001`–`0006` applied (`0004` feature_flags, `0005` income tithe/expense group/savings_goals, `0006` debt_type enum values + `debts.next_due_date` — `0004`–`0006` applied to dev out-of-band via MCP `execute_sql`, which writes **no** ledger row, so `deploy-dev`'s `db push` applies the idempotent files cleanly on merge); **ledger repaired** (`supabase migration repair --status applied 0001 0002`) because manual SQL-Editor application doesn't write `supabase_migrations.schema_migrations` and `0001`/`0002` aren't idempotent. **`0003` (debts + transactions)** was applied via MCP `apply_migration`, which wrote a **timestamp** version that mismatched `deploy-dev`'s `db push` — reconciled by hand (see memory `supabase-mcp-migration-version-mismatch.md`); next push is clean. **Local CLI is linked to dev** (safe — avoids accidental prod pushes; `supabase/.temp/project-ref` = `mlbskwkjwbdewbmoagav`). **Migrations `0007`–`0012`** (transaction FKs/paid · expense→debt link · offering `pct_of_income`+tithe fold · `incomes.is_variable`+`income_overrides` · `debts.start_date` · `savings_goals.type`) are forward-only/idempotent and applied to dev by `deploy-dev` on each merge to master (the mobile/feature overhaul, PRs #25–#31).
...
- **RLS:** enabled on all finance tables (`profiles`, `debts`, `accounts`, `incomes`, `expenses`, `plans`, `plan_runs`, `transactions`, `savings_goals`, **`income_overrides`** — added in `0010`), owner-only policies keyed on `profile_id = auth.uid()`. The **`feature_flags`** table (Phase 1.6) is the exception by design: RLS read-all + **no write policy**, so anyone reads flag state but only service_role/dashboard can flip a flag (verified — customers can't enable hidden features). `transactions` is append-only (no `updated_at`), with indexes on `profile_id` and `debt_id`; `expense_id`/`savings_goal_id` columns exist nullable now but their FKs land in Phase 2 when those tables get UIs.
...
- **`feature_flags` table (BUILT, Phase 1.6):** `key pk, enabled, rollout_pct, audience, updated_at` — interim runtime backend for release flags, flipped from the dashboard/SQL at release sign-off (no redeploy), later superseded by PostHog flags (Track B · B2). In dev the `income`/`expenses`/`savings` flags are currently **on** (flipped by the Phase 2 e2e).

## Feature-flag / entitlement architecture (BUILT — Phase 1.6)
Two-gate gating (full design in the roadmap plan file `~/.claude/plans/let-me-first-ask-hidden-pebble.md`):
- **Release flags:** `lib/flags/registry.ts` (modular flag list) + `lib/flags/provider.ts` (`FlagProvider`, mirrors `lib/llm/provider.ts`) + `lib/flags/supabaseProvider.ts` (reads `feature_flags`, fail-safe to defaults) + `lib/flags/server.ts` (`getFlagProvider`/`featureState`) + `components/flags/FeatureGate.tsx`. Default-off fail-safe.
- **Entitlements:** `lib/entitlements/getEntitlements.ts` + the separate `lib/entitlements/featureAccess.ts` (feature → min tier). Composed via `lib/flags/resolve.ts` → `{ visible, locked }`.
